...
class PromptEnum(str, Enum):
    intent_system = "You are a medical AI assistant that classifies doctor utterances into medical intents. You will receive an explanation for all possible intents. \
There are 20 doctor intents available. Each utterance can contain multiple intents. Respond with true or false for all intents if it is present in the utterance or not. The following is a short description for each intent.\
...
if __name__ == "__main__":
    ...
    if args.llm_setting == "zero_shot":   
        llm_inputs = test_data.text.map(lambda x: prompt.replace("$$SAMPLE$$", x)).to_list()
    elif args.llm_setting == "few_shot":
        llm_inputs = test_data.text.map(lambda x: create_few_shot_candidates(x, args.num_candidates, candidate_data, args.task, token_dict, system_prompt)).to_list()
    ...
    print("Create generator for "+ args.task)
    generator = generate.json(model, Intents) if args.task == "intent" else generate.json(model, Sections)
    ...
    print("Saving")
    test_data["prompts"] = test_data.text.map(lambda x: prompt.replace("$$SAMPLE$$", x))
