...
class MultiTaskBertClassificationModel(pl.LightningModule):
    def __init__(self,
                 tasks: dict = {},
        ...
        self.tasks = tasks
        self.classification_layers = torch.nn.ModuleDict({k:torch.nn.Linear(hidden_dim, v, device=device) for k,v in tasks.items()})
    ...
    def test_step(self, batch, batch_idx, **kwargs):
        ...
        for task_name, num_classes in self.tasks.items():
            logits = self.classification_layers[task_name](encoded)
            test_dict[task_name] = {"logits": logits, "labels": batch["intent_labels"]} if task_name == "intent"\
                else {"logits": logits, "labels": batch["section_labels"]}
    ...
    def on_test_epoch_end(self) -> None:
        ...
        for task_name, num_classes in self.tasks.items():
            ...
            macro_score_auroc = auroc(y_pred, labels, 'multilabel', num_labels=num_classes, average="macro")
            micro_score_auroc = auroc(y_pred, labels, 'multilabel', num_labels=num_classes, average="micro")   
            all_score_auroc = auroc(y_pred, labels, 'multilabel', num_labels=num_classes, average="none")

            macro_score_ap = average_precision(y_pred, labels, 'multilabel', num_labels=num_classes, average="macro")
            micro_score_ap = average_precision(y_pred, labels, 'multilabel', num_labels=num_classes, average="micro")   
            all_score_ap = average_precision(y_pred, labels, 'multilabel', num_labels=num_classes, average="none")
    ...
    def validation_step(self, batch, batch_idx,**kwargs):
        ...
        for task_name, num_classes in self.tasks.items():
            logits = self.classification_layers[task_name](encoded)
            val_dict[task_name] = {"logits": logits, "labels": batch["intent_labels"]} if task_name == "intent"\
                else {"logits": logits, "labels": batch["section_labels"]}
